...
void swap(struct node* ref1, struct node* ref2) {
    ...
    /* Your code here */
    if (!ref2||!ref1) {return;}
    //Case 1: Nodes could be far from each other
  
    if (!(ref1->next==ref2) && !(ref1->prev==ref2))
    {
		struct node* temp = ref1->next;
	...
	//Case 2; Nodes could be next to each other ref2 after ref1
	...
	//Case 3: Nodes could be next to each other ref1 after ref2
	if (ref1->prev ==ref2)
	{
		ref1->prev = ref2->prev;
		if (ref2->prev)
...
struct node* concat(struct node* ref1, struct node* ref2) {
    ...
    if (!ref1) return begin(ref2);
    if (!ref2) return begin(ref1);
    struct node* temp = end(ref1);
	end(ref1)->next = begin(ref2);
	//printf("value of end(ref1): %d, value of begin ref(2): %d\n", value(end(ref1)), value(begin(ref2)));
	begin(ref2)->prev = temp;	
...
int distance(struct node* ref1, struct node* ref2) {
    ...
    int dist = 0;
    /* Your code here */
    //separately checking for ref1 before ref2 and ref2 before ref1
    struct node* temp = ref1;
    //if ref1 is before ref2
...
void minmax(struct node* ref, int* min, int* max) {
	...
	*max = value(ref);
	while (ref->next)
	{
		if (value(ref->next)< *min){
			*min = value(ref->next);
		}
		if (value(ref->next)>*max){
...
bool includes(struct node* ref1, struct node* ref2) {
	...
	while(value(ref2)!=value(ref1) && ref1)
	{
		ref1=ref1->next;
	}
	
	if (value(ref2)==value(ref1))
	{
		while(ref1 && value(ref2->next)==value(ref1->next))
		{	if (ref2->next==end(ref2))
				{return true;}
			ref2=ref2->next;
			ref1=ref1->next;
		}
...
/////////////////////////////////////////////
	...
	int max;
	minmax(list,&min,&max);
	printf("the min value from list: %d, the max value from list: %d \n",min,max );
